...
public abstract class BaseConstructFormatter<T> {
    ...
    /**
     * Format items for constructs that need grouping (like EXTENDS with many modules).
     * Groups items to fit multiple per line when possible.
     */
    protected Doc formatWithGrouping(List<T> items, String prefix, Function<T, Doc> itemFormatter,
                                   int maxItemsPerGroup) {
        
        if (items.size() <= 3) {
            // For short lists, use simple formatting
            return formatAsSingleLine(items, prefix, itemFormatter);
        }
        
        // Build groups of items
        Doc result = Doc.text(prefix).append(itemFormatter.apply(items.get(0)));
        
        for (int i = 1; i < items.size(); i++) {
            if (i % maxItemsPerGroup == 0) {
                // Start new line for new group
                result = result
                        .append(Doc.text(","))
                        .appendLine(itemFormatter.apply(items.get(i)).indent(prefix.length()));
            } else {
                // Continue on same line within group
                result = result.append(Doc.text(", ")).append(itemFormatter.apply(items.get(i)));
            }
        }
