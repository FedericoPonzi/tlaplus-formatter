...
ASSUME
       HostMappingType ==
                          ...
                          /\ \A mapping \in HostMapping: /\ Cardinality(mapping) = 3
...
QueryReplyMessageType ==
                         "QueryReplyMessageType"

TerminationMessageType ==
                          "TerminationMessageType"

QueryMessage ==
                [type:{QueryMessageType},src:SlushLoopProcess,dst:SlushQueryProcess,color:Color]

QueryReplyMessage ==
                     [type:{QueryReplyMessageType},src:SlushQueryProcess,dst:SlushLoopProcess,color:Color]
...
PendingQueryMessage(pid) ==
                            { m \in message: /\ m.type = QueryMessageType
                                             /\ m.dst = pid }

PendingQueryReplyMessage(pid) ==
                                 { m \in message: /\ m.type = QueryReplyMessageType
                                                  /\ m.dst = pid }
...
vars ==
        <<pick, message, pc, sampleSet, loopVariant>>

ProcSet ==
           (SlushQueryProcess) \cup (SlushLoopProcess) \cup {"ClientRequest"}
...
QueryReplyLoop(self) ==
                        ...
                        /\ IF
                                ~Terminate
                           THEN
                                /\ pc' = [pc EXCEPT ![self]="WaitForQueryMessageOrTermination"]
                           ELSE
...
WaitForQueryMessageOrTermination(self) ==
                                          ...
                                          /\ IF
                                             ...
                                             ELSE
                                                  /\ pc' = [pc EXCEPT ![self]="RespondToQueryMessage"]
                                          /\ UNCHANGED <<pick, message, sampleSet, loopVariant>>

RespondToQueryMessage(self) ==
                               /\ pc[self] = "RespondToQueryMessage"
                               /\ \E msg \in PendingQueryMessage(self): LET
                                                                            color ==
                                                                                     IF
                                                                                          Pick(self) = NoColor
                                                                                     THEN
...
ExecuteSlushLoop(self) ==
                          ...
                          /\ IF
                                  loopVariant[self] < SlushIterationCount
                             THEN
                                  /\ pc' = [pc EXCEPT ![self]="QuerySampleSet"]
                             ELSE
                                  /\ pc' = [pc EXCEPT ![self]="SlushLoopTermination"]
                          /\ UNCHANGED <<pick, message, sampleSet, loopVariant>>
...
TallyQueryReplies(self) ==
                           ...
                           /\ LET
                                  redTally ==
                                              Cardinality({ msg \in PendingQueryReplyMessage(self): /\ msg.src \in sampleSet[self]
                                                                                                    /\ msg.color = Red })
                              IN
                                  LET
                                      blueTally ==
                                  ...
                                  IN
                                      IF
                                           redTally >= PickFlipThreshold
                                      THEN
                                           /\ pick' = [pick EXCEPT ![HostOf[self]]=Red]
                                      ELSE
                                           /\ IF
                                                   blueTally >= PickFlipThreshold
                                              THEN
...
SlushLoop(self) ==
                   RequireColorAssignment(self) \/ ExecuteSlushLoop(self) \/ QuerySampleSet(self) \/ TallyQueryReplies(self) \/ SlushLoopTermination(self)

ClientRequestLoop ==
                     /\ pc["ClientRequest"] = "ClientRequestLoop"
                     /\ IF
                             \E n \in Node: pick[n] = NoColor
                        THEN
                             /\ pc' = [pc EXCEPT !["ClientRequest"]="AssignColorToNode"]
                        ELSE
...
AssignColorToNode ==
                     ...
                     /\ \E node \in Node: \E color \in Color: IF
                                                                   pick[node] = NoColor
                                                              THEN
...
Next ==
        ClientRequest \/ (\E self \in SlushQueryProcess: SlushQuery(self)) \/ (\E self \in SlushLoopProcess: SlushLoop(self)) \/ Terminating
