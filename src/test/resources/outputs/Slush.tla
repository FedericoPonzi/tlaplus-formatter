...
HostOf[pid \in SlushLoopProcess \cup SlushQueryProcess] ==
  CHOOSE n \in Node: /\ \E mapping \in HostMapping: /\ n \in mapping
                                                    /\ pid \in mapping
...
SlushQuery(self) ==
  QueryReplyLoop(self) \/ WaitForQueryMessageOrTermination(self) \/
    RespondToQueryMessage(self)
...
SlushLoopTermination(self) ==
  ...
  /\ message' =
       ( message \cup { [ type |-> TerminationMessageType, pid |-> self ] } )
